...
const RelatedDoctors = ({drId, speciality}: { drId: string, speciality: string}) => {
  const {doctors} = useDoctorStore();
  const navigate = useNavigate();

  const [relatedDoctors, setRelatedDoctors] = useState<TDoctor[]>([]);

  useEffect(() => {
    const filteredDoctors = doctors.filter((doctor) => doctor.speciality === speciality && doctor._id !== drId);
    setRelatedDoctors(filteredDoctors);
  }, [doctors, drId, speciality]);
